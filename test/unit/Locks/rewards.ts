...
describe('rewards', function () {
  ...
  it('only vault manager can add rewards', async function () {
  ...
  it('vault manager should be able to add rewards', async function () {
    const { accounts: {members, vaultManager}, contracts: {locks, registry, rwaSharesMock} } = await networkHelpers.loadFixture(setupFixture);
    ...
    await locks.connect(vaultManager).addReward(memberIds, assetAmounts, await rwaSharesMock.getAddress(), totalAssetAmounts, snapshotTimestamp);
  ...
  it('total rewards should be equal to total asset amounts', async function () {
    const { accounts: {vaultManager}, contracts: {locks, rwaSharesMock} } = await networkHelpers.loadFixture(setupFixture);
    ...
    await expect(locks.connect(vaultManager).addReward(memberIds, assetAmounts, await rwaSharesMock.getAddress(), parseUsdc("5000"), 0))
